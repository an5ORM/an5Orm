...
async function pull() {
    ...
    const tables = await (await getDb()).$queryRawUnsafe(`
  ...
  `);
    ...
    const fileContents = {};
    const modelToFileMap = {}; // modelName -> fileName
    const tableToFileMap = {}; // tableName -> fileName
    if (fs_1.default.existsSync(schemaDir)) {
        const files = fs_1.default.readdirSync(schemaDir).filter(f => f.endsWith(".an5"));
        for (const file of files) {
            const fullPath = path_1.default.join(schemaDir, file);
    ...
    for (const table of tables) {
        ...
        // Get columns
        const columns = await (await getDb()).$queryRawUnsafe(`
      SELECT 
    ...
    `, tableName);
        // Get indexes
        const indexes = await (await getDb()).$queryRawUnsafe(`
      SELECT 
          i.name AS indexName,
    ...
    `, tableName);
        ...
        if (fileContent) {
            ...
            if (match) {
                ...
                for (let line of lines) {
                    line = line.trim();
                    if (!line || line.startsWith("@@") || line.startsWith("model ") || line === "}")
                        continue;
        ...
        for (const col of columns) {
            ...
            if (uniqueColumns.has(columnName) && !isPrimaryKey)
                attributes += " @unique";
            if (defaultValue) {
                if (defaultValue.includes("newid()")) {
                    ...
                    if (dt === "bit") {
                        ...
                        else if (cleaned === "0" || cleaned.toLowerCase() === "false")
                            attributes += " @default(false)";
                    }
                    else if (dt === "int" || dt === "float" || dt === "bigint" || dt === "decimal" || dt === "numeric") {
                        if (!isNaN(Number(cleaned)))
                            attributes += ` @default(${cleaned})`;
                    }
    ...
    try {
        const { indexSchema, indexQuerySamples } = require(path_1.default.join(rootDir, "an5Agent", "dist", "rag", "indexer.js"));
