...
`;
        fs_1.default.writeFileSync(path_1.default.join(this.outputDir, 'base.ts'), baseTs);
    }
    generateModelFile(model) {
        ...
        fs_1.default.writeFileSync(path_1.default.join(this.outputDir, `${model.name}.ts`), content);
    }
    generateIndexTs(models) {
        ...
        content += `}\n`;
        fs_1.default.writeFileSync(path_1.default.join(this.outputDir, 'index.ts'), content);
    }
    ...
    getFieldFilterType(field) {
        const normalizedType = this.normalizeType(field.type);
        if (normalizedType === 'string')
            return field.isOptional ? 'An5.StringNullableFilter' : 'An5.StringFilter';
        if (normalizedType === 'number')
            return field.isOptional ? 'An5.NumberNullableFilter' : 'An5.NumberFilter';
        if (normalizedType === 'boolean')
            return field.isOptional ? 'An5.BooleanNullableFilter' : 'An5.BooleanFilter';
        if (normalizedType === 'Date')
            return field.isOptional ? 'An5.DateTimeNullableFilter' : 'An5.DateTimeFilter';
        return 'any';
    }
    normalizeType(type) {
